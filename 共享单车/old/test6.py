...
for layer in range(len(global1)):
    ...
    for i in range(len(x)):
        if i != idx:
            ...
            currentAxis = plt.gca()
            rect = patches.Rectangle((x[i] + 0.05, y[i]), 0.15, 0.2, linewidth=1, edgecolor='r', facecolor='none')
            currentAxis.add_patch(rect)
            plt.annotate((str(global1[layer][4])), (x[i], y[i]), xytext=(x[i] + 0.1, y[i] + 0.04))
        plt.annotate((i + 1), (x[i], y[i]), xytext=(x[i] - 0.01, y[i] + 0.1))
        currentAxis = plt.gca()
        rect = patches.Rectangle((x[i] - 0.1, y[i] - 0.43), 0.3, 0.35, linewidth=1, edgecolor='r', facecolor='none')
        currentAxis.add_patch(rect)
        ...
        ax.set_xlabel(('T = ' + str(global1[layer][0]), 'Q = ' + str(global1[layer][1]),
                       # 'Car = ' + str(global1[layer][2]), 'At = ' + str(global1[layer][3]),
                       # 'Ci = ' + str(global1[layer][4])))
                       ))
    plt.axis([1, 3, 0, 3.5])
...
def update(times):
    ...
    plt.plot([x[path[times] - 1], x[path[times + 1] - 1]], [y[path[times] - 1], y[path[times + 1] - 1]], color='r')
    # plt.annotate(' ', (x[path[times] - 1], y[path[times] - 1]))
    plt.annotate(path[times + 1], (x[path[times + 1] - 1], y[path[times + 1] - 1]))
    ax.set_xlabel(('T = ' + str(global1[times + 1][0]), 'Q = ' + str(global1[times + 1][1]),
                   'Car = ' + str(global1[times + 1][2]), 'At = ' + str(global1[times + 1][3]),
